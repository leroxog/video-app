...
def test_sevenai_page_requires_login(client, raw_client):
    assert raw_client.get("/7ai", follow_redirects=False).status_code in (302, 401)
    register(client)
    assert client.get("/7ai").status_code == 200


def test_ai_chats_scoped_by_character(client, monkeypatch):
    # 2026-09-08: Nex and 7Ai are two separate characters with separate
    # chat histories (AiChat.character) -- a chat started on one must never
    # show up in the other's sidebar/list, even for the same user.
    import ai_assistant

    monkeypatch.setattr(
        ai_assistant, "generate_reply",
        lambda *a, **k: ("Antwort", None),
    )
    register(client)

    nex_res = client.post("/api/ai/chat", json={"message": "Hallo Nex"})
    nex_chat_id = nex_res.get_json()["chat_id"]

    sevenai_res = client.post(
        "/api/ai/chat", json={"message": "Hallo 7Ai", "project_type": "sevenai", "character": "sevenai"},
    )
    sevenai_chat_id = sevenai_res.get_json()["chat_id"]
    assert sevenai_chat_id != nex_chat_id

    from models import AiChat
    assert db.session.get(AiChat, nex_chat_id).character == "nex"
    assert db.session.get(AiChat, sevenai_chat_id).character == "sevenai"

    nex_chats = client.get("/api/ai/chats?character=nex").get_json()["chats"]
    assert [c["id"] for c in nex_chats] == [nex_chat_id]

    sevenai_chats = client.get("/api/ai/chats?character=sevenai").get_json()["chats"]
    assert [c["id"] for c in sevenai_chats] == [sevenai_chat_id]

    # No `character` query param at all defaults to "nex" -- keeps the
    # endpoint backward-compatible with any caller that doesn't send it.
    default_chats = client.get("/api/ai/chats").get_json()["chats"]
    assert [c["id"] for c in default_chats] == [nex_chat_id]
...
def test_sevenai_uses_its_own_system_prompt_and_tools(monkeypatch):
    # 2026-09-08: 7Ai is a second, deliberately blunter/sassier character --
    # its own top-level system prompt (not built on Nex's GENERAL_SYSTEM_PROMPT/
    # FRIEND_CHARACTER_ADDENDUM), its own tool set (no remember_user_fact/
    # adjust_personality_trait -- 7Ai has no per-user memory or tunable
    # personality), routed through _call_model_with_router just like Nex's
    # general mode (not the embedded tool_call convention game/webapp/code use).
    import ai_assistant
    ...
    def fake_call_model(*args, **kwargs):
        captured["called_call_model_directly"] = True
        return "ok", None
    ...
    assert "called_call_model_directly" not in captured
    assert captured["tools"] == ai_assistant.SEVENAI_TOOLS
    assert ai_assistant.REMEMBER_USER_FACT_TOOL not in captured["tools"]
    assert ai_assistant.ADJUST_PERSONALITY_TOOL not in captured["tools"]
    assert captured["system_prompt"] == (
