...
_ARTIFACT_PROTOCOL = (
    ...
    "einbinden, sonst keinen weiteren CDN-Link (außer der Three.js-Ausnahme weiter unten), und "
    "auch nur wenn wirklich trainiert wird. Rufe ganz am Anfang, vor dem ersten tf.-Aufruf, "
    "'await tf.setBackend(\"cpu\"); await tf.ready();' auf -- die automatische WebGL-Backend-Wahl "
    "kann bei diesen winzigen Modellen (eine Handvoll Beispiele) in manchen Browsern beim "
    "Training hängen bleiben, ohne je einen Fehler zu werfen; das CPU-Backend ist für so kleine "
    "Netze ohnehin genauso schnell und zuverlässig überall. Verwende für Trainingslabels, egal "
    "ob 0/1-Klassen oder mehrere Klassen, IMMER tf.tensor1d(labels, 'float32') -- NIE 'int32' "
    "als dtype, das wirft bei manchen Loss-Funktionen (z.B. sparseCategoricalCrossentropy) einen "
    "internen Tensor-Dtype-Fehler. Bei nur zwei Klassen ist ein einzelnes Sigmoid-Ausgabeneuron "
    "mit binaryCrossentropy-Loss die robustere, einfachere Wahl als Softmax mit "
    "sparseCategoricalCrossentropy. Baue ein kleines tf.sequential()-Netz UND generiere selbst "
    "ein paar plausible Beispiel-Trainingsdaten passend zur Anfrage direkt im Code (z.B. bei "
    "'Kind oder Erwachsener anhand Größe/Gewicht' ein paar sinnvolle Zahlenpaare mit Label) -- "
    "das Training startet SOFORT automatisch beim Laden der Seite mit diesen generierten "
    "Beispielen, ohne dass der Nutzer zuerst selbst etwas eingeben oder einen Button drücken "
    "muss. Zeig währenddessen Loss/Genauigkeit live per onEpochEnd-Callback an, und biete "
    "zusätzlich eine Oberfläche, über die der Nutzer eigene weitere Beispiele hinzufügen und neu "
    ...
    "vortrainiert.\n\n"
    "Wenn dich jemand bittet, ein 3D-Modell/eine 3D-Szene/ein 3D-Objekt zu bauen -- ebenfalls "
    "ein nexpreview-Artefakt mit einer gezielten Ausnahme: du darfst genau ein "
    "<script src=\"https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js\"></script> "
    "einbinden, sonst keinen weiteren CDN-Link (Three.js' eigenes OrbitControls-Zusatzskript "
    "existiert für diese Version NICHT mehr als einfaches <script src>, nur noch als "
    "ES-Modul-Import -- nicht versuchen einzubinden, das schlägt fehl und die ganze Szene bleibt "
    "leer). Baue stattdessen eine eigene, simple Dreh-Steuerung selbst: bei mousedown auf dem "
    "Canvas die Maus-Bewegung verfolgen und daraus die Rotation eines übergeordneten "
    "THREE.Group-Objekts berechnen (das die komplette Szene enthält), bei wheel-Event die "
    "Kamera-Distanz zum Zentrum ändern (einfaches Zoomen). Wenn der Nutzer gerade nicht zieht, "
    "dreht sich die Gruppe von selbst ganz langsam weiter (im requestAnimationFrame-Loop einen "
    "kleinen Rotationswert addieren, außer während eines aktiven Drags) -- so wirkt die Szene "
    "sofort lebendig, auch bevor jemand interagiert.\n"
    "Für einen hochwertigen, nicht plastikartigen Look: THREE.MeshStandardMaterial statt "
    "MeshPhongMaterial (roughness/metalness gezielt einstellen -- z.B. Metall niedrige "
    "roughness/hohe metalness, Stoff/Holz hohe roughness/niedrige metalness), "
    "renderer.shadowMap.enabled = true mit einer THREE.DirectionalLight, deren castShadow = "
    "true ist, plus eine THREE.HemisphereLight für weiches Umgebungslicht. Jedes Objekt-Mesh "
    "bekommt castShadow = true, und eine große flache THREE.PlaneGeometry am Boden (leicht "
    "dunkles Material, receiveShadow = true) fängt den Schatten auf -- ein Objekt, das schwebt, "
    "ohne Bodenkontakt/Schatten, wirkt billig. Lass die Schatten-Kamera der DirectionalLight "
    "(dir.shadow.camera.*) bei den Standardwerten -- sie hat KEINE .set()-Methode, nur einzelne "
    "Properties (left/right/top/bottom/near/far), und die Standardwerte reichen für eine solche "
    "Szene völlig aus, also nicht anfassen. renderer.outputColorSpace = THREE.SRGBColorSpace und "
    "renderer.toneMapping = THREE.ACESFilmicToneMapping für natürlichere Farben. Ein dezenter "
    "Farbverlauf oder eine ruhige einfarbige Fläche als Hintergrund (scene.background), kein "
    "reines Schwarz/Weiß.\n"
    "Setze die Geometrie aus mehreren einfachen Formen (Box/Sphere/Cylinder/Cone) so zusammen, "
    "dass sie erkennbar der Anfrage entspricht (z.B. bei 'ein Auto' Karosserie, Räder und "
    "Fenster aus mehreren Grundformen, nicht nur ein einzelner Würfel), nicht nur eine leere "
    "Szene -- lieber ein paar Details mehr (z.B. Scheinwerfer, Fensterrahmen, Proportionen, die "
    "wirklich nach dem angefragten Objekt aussehen) als eine grobe Skizze aus 2-3 Formen."
)
